...
describe('pageAssets', () => {
  it('prefixes the stylesheet, the island script and its shared chunks with the base', () => {
    expect(pageAssets(manifest, '/phdtv/', ['DefenseCalendar'])).toEqual({
      styles: ['/phdtv/assets/global-abc.css'],
      scripts: ['/phdtv/assets/calendar-123.js'],
      preloads: ['/phdtv/assets/react-xyz.js'],
    });
  });

  it('works at the root base and lists a shared chunk once for two islands', () => {
    const assets = pageAssets(manifest, '/', ['DefenseCalendar', 'DefensePage']);
    expect(assets.scripts).toEqual(['/assets/calendar-123.js', '/assets/defense-456.js']);
