...
The branch was fast-forwarded into `main` at the end of the session and its worktree removed. `main` is nine commits ahead of `origin/main` and unpushed, so the page goes live with the next push. The calendar-views worktree has uncommitted rewrites of `Shell`, `PageIntro`, `pages.tsx` and the stylesheet; it will resolve a one-line conflict in each of the nav, the page functions and the end of the stylesheet when it lands.

> **Update 2026-09-06**: the calendar branch landed as a merge commit that evening, carrying the about page's links, route and page across; `main` was then pushed and deployed. See [the integration entry](2026-09-06-06-integration-and-deploy.md).
